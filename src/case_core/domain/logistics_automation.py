...
class LogisticsAutomationPolicy(AutomationPolicy):
    """Automation policy for logistics domain with deterministic risk assessment."""
    ...
    def assess_risk(
        ...
        if validation_status != "valid":
            ...
            policy_violations.append("schema_or_semantic_validation_failure")
        ...
        requires_hitl = self._requires_hitl(risk_level, factors, confidence)
        automation_decision = self._decide_automation(risk_level, factors, requires_hitl, policy_violations)
    ...
    def _requires_hitl(self, risk_level: RiskLevel, factors: list[str], confidence: float) -> bool:
        if risk_level in (RiskLevel.HIGH, RiskLevel.CRITICAL):
            return True
        if confidence < 0.6:
            return True
        if "evidence_insufficient" in factors:
            return True
        return False
